...
class VajraFlywheel:
    ...
    def get_improvement_recommendations(self, top_k: int = 5) -> List[Improvement]:
        ...
        # Sort by expected gain (handles both minimize and maximize targets)
        valid_improvements.sort(
            key=lambda x: self._expected_gain(x) * x.confidence,
            reverse=True,
        )
    ...
    def optimize_tradeoff(
        ...
        valid_configs: List[PerformanceSnapshot] = []
        for snapshot in self.performance_history:
            if self._meets_constraints(snapshot, constraints):
                valid_configs.append(snapshot)
        
        if not valid_configs:
            return {"error": "No configuration meets constraints"}
        
        # Find best for primary target (minimize vs maximize)
        if self._is_higher_better(primary_target):
            best = max(valid_configs, key=lambda s: self._metric_value(s, primary_target))
        else:
            best = min(valid_configs, key=lambda s: self._metric_value(s, primary_target))
    ...
    def _analyze_for_improvements(self):
        ...
        for target in self.optimization_targets:
            values = [self._metric_value(s, target) for s in recent]
            ...
            # Detect worsening performance (direction depends on target)
            if self._is_worsening_trend(target, trend):
                potential = self._potential_value(target, current_avg)
                ...
                improvement = Improvement(
                    ...
                    expected_impact=self._format_expected_impact(target, current_avg, potential),
                )
    ...
    def _metric_value(self, snapshot: PerformanceSnapshot, target: OptimizationTarget) -> float:
        """Map optimization targets to snapshot fields."""
        if target == OptimizationTarget.LATENCY:
            return snapshot.latency_ms
        if target == OptimizationTarget.COST:
            return snapshot.cost_per_query
        if target == OptimizationTarget.ACCURACY:
            return snapshot.accuracy_score
        if target == OptimizationTarget.THROUGHPUT:
            return snapshot.throughput_qps
        if target == OptimizationTarget.RELIABILITY:
            return snapshot.reliability_score
        return 0.0

    def _is_higher_better(self, target: OptimizationTarget) -> bool:
        return target in {
            OptimizationTarget.ACCURACY,
    ...
    def _is_worsening_trend(self, target: OptimizationTarget, trend: float) -> bool:
        """Return True if recent trend indicates degradation."""
        # For minimize targets (latency/cost), increasing trend is worse.
        if not self._is_higher_better(target):
            return trend > 0.05
        # For maximize targets, decreasing trend is worse.
        return trend < -0.05

    def _potential_value(self, target: OptimizationTarget, current_avg: float) -> float:
        """Compute a conservative 'reachable' potential target from current_avg."""
        if self._is_higher_better(target):
            if target in {OptimizationTarget.ACCURACY, OptimizationTarget.RELIABILITY}:
                return min(current_avg * 1.1, 1.0)
            return current_avg * 1.1
        # Minimize targets
        return current_avg * 0.9

    def _format_expected_impact(self, target: OptimizationTarget, current: float, potential: float) -> str:
        if self._is_higher_better(target):
            return f"Increase {target.value} by {potential - current:.4f}"
        return f"Reduce {target.value} by {current - potential:.4f}"

    def _expected_gain(self, improvement: Improvement) -> float:
        if self._is_higher_better(improvement.target):
            return improvement.potential_value - improvement.current_value
        return improvement.current_value - improvement.potential_value
    ...
    def _meets_constraints(
        ...
        for target, threshold in constraints.items():
            val = self._metric_value(snapshot, target)
            if self._is_higher_better(target):
                if val < threshold:
                    return False
            else:
