...
def test_shadow_security_modules_execute():
    ...
    assert profile.redteam_summary.get("status") == "ok"
...
def test_shadow_loop_runner_writes_outputs(tmp_path):
    ...
    assert summary["trend"]["baseline"] is True

    second = run_loop(tmp_path)
    assert second["trend"]["baseline"] is False
    assert "gate_stats" in second
